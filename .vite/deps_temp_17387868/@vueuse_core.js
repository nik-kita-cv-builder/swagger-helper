...
// node_modules/.deno/vue-demi@0.14.7/node_modules/vue-demi/lib/index.mjs
var isVue2 = false;
var isVue3 = true;
...
// node_modules/.deno/@vueuse+shared@10.9.0/node_modules/@vueuse/shared/index.mjs
function computedEager(fn, options) {
  ...
  watchEffect(() => {
    ...
    ...options,
    flush: (_a = options == null ? void 0 : options.flush) != null
      ? _a
      : "sync",
  });
...
function createInjectionState(composable, options) {
  const key = (options == null ? void 0 : options.injectionKey) ||
...
function throttleFilter(...args) {
  ...
  if (!isRef(args[0]) && typeof args[0] === "object") {
    ({ delay: ms, trailing = true, leading = true, rejectOnCancel = false } =
      args[0]);
  } else {
...
function toRefs2(objectRef, options = {}) {
  ...
  for (const key in objectRef.value) {
    result[key] = customRef(() => ({
      ...
      set(v) {
        var _a;
        const replaceRef = (_a = toValue(options.replaceRef)) != null
          ? _a
          : true;
        if (replaceRef) {
...
function createUntil(r, isNot = false) {
  ...
  function toBe(value, options) {
    ...
    const { flush = "sync", deep = false, timeout, throwOnTimeout } =
...
function useArrayDifference(...args) {
  ...
  return computed(() =>
    toValue(list).filter((x) =>
      toValue(values).findIndex((y) => compareFn(x, y)) === -1
    )
  );
}
...
// node_modules/.deno/@vueuse+core@10.9.0/node_modules/@vueuse/core/index.mjs
  ...
  const reuse = defineComponent({
    ...
    setup(_, { attrs, slots }) {
      return () => {
        ...
        const vnode = (_a = render.value) == null
          ? void 0
...
function onLongPress(target, handler, options) {
  ...
  function onDown(ev) {
    ...
    if (
      ((_a2 = options == null ? void 0 : options.modifiers) == null
        ? void 0
        : _a2.self) && ev.target !== elementRef.value
    ) {
      return;
    }
    clear();
  ...
  const listenerOptions = {
    capture: (_a = options == null ? void 0 : options.modifiers) == null
      ? void 0
      : _a.capture,
    once: (_b = options == null ? void 0 : options.modifiers) == null
      ? void 0
      : _b.once,
  };
  const cleanup = [
  ...
  const element = customRef((track, trigger) => {
    ...
    return {
      get() {
        var _a, _b;
        track();
        return (_b = (_a = instance == null ? void 0 : instance.proxy) == null
            ? void 0
            : _a.$refs[key]) != null
          ? _b
          : initialValue;
...
function useActiveElement(options = {}) {
  ...
  const document2 = (_a = options.document) != null
    ? _a
    : window2 == null
    ? void 0
    : window2.document;
  const getDeepActiveElement = () => {
    ...
    if (deep) {
      while (element == null ? void 0 : element.shadowRoot) {
        element = (_a2 = element == null ? void 0 : element.shadowRoot) == null
          ? void 0
          : _a2.activeElement;
      }
    }
    ...
    return prev.then((prevRes) => {
      ...
      if (
        ((_a = result[activeIndex.value]) == null ? void 0 : _a.state) ===
          promiseState.rejected && interrupt
      ) {
        onFinished();
        return;
      }
...
function useBluetooth(options) {
  ...
  async function requestDevice() {
    ...
    try {
      device.value =
        await (navigator == null ? void 0 : navigator.bluetooth.requestDevice({
          acceptAllDevices,
          filters,
          optionalServices,
        }));
...
function useClipboard(options = {}) {
  ...
  function legacyRead() {
    var _a, _b, _c;
    return (_c =
        (_b = (_a = document == null ? void 0 : document.getSelection) == null
            ? void 0
...
function useStorage(key, defaults2, storage, options = {}) {
  ...
  const serializer = (_a = options.serializer) != null
    ? _a
  ...
  const updateHTMLAttrs = getSSRHandler(
    ...
    (selector2, attribute2, value) => {
      ...
      if (attribute2 === "class") {
        ...
        Object.values(modes).flatMap((i) => (i || "").split(/\s/g)).filter(
          Boolean,
        ).forEach((v) => {
...
function useMutationObserver(target, callback, options = {}) {
  ...
  const targets = computed(() => {
    ...
    const items = (Array.isArray(value) ? value : [value]).map(unrefElement)
      .filter(notNullish);
...
function useCycleList(list, options) {
  ...
  const index = computed({
    get() {
      ...
      if (index2 < 0) {
        index2 = (_a = options == null ? void 0 : options.fallbackIndex) != null
          ? _a
          : 0;
      }
      return index2;
    ...
    useEventListener(target, "dragenter", (event) => {
      ...
      const types = Array.from(
        ((_a = event == null ? void 0 : event.dataTransfer) == null
          ? void 0
          : _a.items) || [],
      ).map((i) => i.kind === "file" ? i.type : null).filter(notNullish);
...
function useElementSize(
  ...
  tryOnMounted(() => {
    ...
    if (ele) {
      ...
      height.value = "offsetHeight" in ele
        ? ele.offsetHeight
        : initialSize.height;
    }
  });
...
function useIntersectionObserver(target, callback, options = {}) {
  ...
  const targets = computed(() => {
    const _target = toValue(target);
    return (Array.isArray(_target) ? _target : [_target]).map(unrefElement)
      .filter(notNullish);
  });
  let cleanup = noop;
...
function useElementVisibility(element, options = {}) {
  const { window: window2 = defaultWindow, scrollTarget, threshold = 0 } =
    options;
...
function useFetch(url, ...args) {
  ...
  function waitUntilFinished() {
    return new Promise((resolve, reject) => {
      until(isFinished).toBe(true).then(() => resolve(shell)).catch((error2) =>
        reject(error2)
      );
    });
  }
...
function useFileSystemAccess(options = {}) {
  ...
  const isSupported = useSupported(() =>
    window2 && "showSaveFilePicker" in window2 &&
  ...
  async function updateFile() {
    var _a;
    file.value =
      await ((_a = fileHandle.value) == null ? void 0 : _a.getFile());
  }
  async function updateData() {
    ...
    if (type === "Text") {
      ...
      data.value =
        await ((_b = file.value) == null ? void 0 : _b.arrayBuffer());
    } else if (type === "Blob") {
...
function useFps(options) {
  ...
  const every = (_a = options == null ? void 0 : options.every) != null
    ? _a
    : 10;
...
function useFullscreen(target, options = {}) {
  ...
  const requestMethod = computed(() => {
    return [
      ...
      document2 && m in document2 || targetRef.value && m in targetRef.value
    );
  });
  ...
  const isSupported = useSupported(() =>
    targetRef.value && document2 && requestMethod.value !== void 0 &&
    exitMethod.value !== void 0 && fullscreenEnabled.value !== void 0
  );
...
function useGamepad(options = {}) {
  ...
  const stateFromGamepad = (gamepad) => {
    ...
    const vibrationActuator = "vibrationActuator" in gamepad
...
function useScroll(element, options = {}) {
  ...
  const setArrivedState = (target) => {
    ...
    const right = Math.abs(scrollLeft) + el.clientWidth >=
    ...
    const bottom = Math.abs(scrollTop) + el.clientHeight >=
...
function useInfiniteScroll(element, onLoadMore, options = {}) {
  ...
  function checkAndLoad() {
    state.measure();
    if (
      !observedElement.value || !isElementVisible.value ||
...
function useMediaControls(target, options = {}) {
  ...
  const supportsPictureInPicture = document2 &&
    "pictureInPictureEnabled" in document2;
  ...
  if (window2) {
    stop = watch(
      ...
      () => {
        ...
        elementPositionX.value = left +
          (type === "page" ? window2.pageXOffset : 0);
        elementPositionY.value = top +
          (type === "page" ? window2.pageYOffset : 0);
        elementHeight.value = height;
        elementWidth.value = width;
        const elX = x.value - elementPositionX.value;
        const elY = y.value - elementPositionY.value;
        isOutside.value = width === 0 || height === 0 || elX < 0 || elY < 0 ||
...
function usePointer(options = {}) {
  ...
  const handler = (event) => {
    isInside.value = true;
    if (
      options.pointerTypes && !options.pointerTypes.includes(event.pointerType)
    ) {
      return;
    }
...
function usePointerLock(target, options = {}) {
  ...
  if (isSupported.value) {
    useEventListener(document2, "pointerlockchange", () => {
      var _a;
      const currentElement = (_a = document2.pointerLockElement) != null
        ? _a
        : element.value;
...
function usePointerSwipe(target, options = {}) {
  ...
  const eventIsAllowed = (e) => {
    ...
    return (_c = (_b = (_a = options.pointerTypes) == null
          ? void 0
...
function useSpeechRecognition(options = {}) {
  ...
  const SpeechRecognition = window2 &&
...
function useTextareaAutosize(options) {
  ...
  const styleProp = (_a = options == null ? void 0 : options.styleProp) != null
    ? _a
    : "height";
  ...
  function triggerResize() {
    ...
    textareaScrollHeight.value = (_a2 = textarea.value) == null
      ? void 0
      : _a2.scrollHeight;
...
function formatTimeAgo(from, options = {}, now2 = Date.now()) {
  ...
  if (typeof max === "string") {
    const unitMax = (_a = units.find((i) => i.name === max)) == null
      ? void 0
      : _a.max;
...
function useVModel(props, key, emit, options = {}) {
  ...
  if (!key) {
    if (isVue2) {
      const modelOptions = (_e = (_d = vm == null ? void 0 : vm.proxy) == null
          ? void 0
          : _d.$options) == null
        ? void 0
        : _e.model;
...
function createCalculateRange(
  ...
  return () => {
    ...
    if (element) {
      ...
      currentList.value = source.value.slice(state.value.start, state.value.end)
        .map((ele, index) => ({
          data: ele,
...
function createGetDistance(itemSize, source) {
  return (index) => {
    ...
    const size = source.value.slice(0, index).reduce(
      (sum, _, i) => sum + itemSize(i),
      0,
    );
    return size;
  };
...
function useWebNotification(options = {}) {
  ...
  const permissionGranted = ref(
    isSupported.value && "permission" in Notification &&
...
function jobRunner(userFunc) {
  return (e) => {
    ...
    return Promise.resolve(userFunc.apply(void 0, userFuncArgs)).then(
      (result) => {
